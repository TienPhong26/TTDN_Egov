...
@section('title', 'Phê duyệt văn bản nôi bộ')
...
@section('content')
    ...
    <div id="page-wrapper">
        <div class="container-fluid">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Phê duyệt văn bản nôi bộ</h1>
                </div>
            </div>
            ...
            <form action="{{ route('admin.vanbannoibo.pheduyetnoibo', ['id' => $vanbannoibo->id]) }}" method="POST">
                {{ csrf_field() }}
            
                <input type="hidden" name="vanban_id" value="{{ $vanbannoibo->id }}">
                <table style="border-collapse: collapse; width: 80%;">
                    <tr>
                        <td>  <div class="form-group">
                            <label>Số Hiệu Đi:</label>
                            <input type="text" class="form-control" style="width: 400px;" value="{{ $vanbannoibo->so_hieudi }}" readonly>
                        </div></td>
                      
                            @if(is_iterable($user))
                                            @foreach($user as $us)
                                                @if($us->id == $vanbannoibo->nguoiky)
                                                    <td>{{ $us->name }}</td>
                                                @endif
                                            @endforeach
                                        @else
                    ...
                    <tr>
                        <td colspan="3">
                            <div class="form-group">
                                <label>Ngày ký:</label>
                                <input style="width: 100px;" type="text" class="form-control" value="{{ $vanbannoibo->ngayky}}" readonly>
                            </div>
                        </td>
                    ...
                    <tr>
                        <td>
                            <div class="form-group">
                                ...
                                <textarea class="form-control " style="width: 400px;" rows="3" readonly >{{ $vanbannoibo->trichyeu }}</textarea>
                                {{-- <input type="text" class="form-control" value="{{ $vanbannoibo->trich_yeu }}" readonly> --}}
                            </div>
                        </td>
                        <td>
                            <div class="form-group">
                                <label>Ghi chú:</label>
                                <textarea class="form-control " style="width: 400px;" rows="3" readonly >{{ $vanbannoibo->ghichu }}</textarea>
                            </div>
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <div class="form-group">
                                ...
                                <a href="{{ asset($vanbannoibo->ten_file) }}" >{{ $vanbannoibo->ten_file}}</a>
                            </div>            
                        </td>
                    </tr>
                </table>
                ...
                <a href="{{ url('admin/vanbannoibo/pheduyet') }}" class="btn btn-default" class="btn btn-primary"><i class="fa-solid fa-backward"></i> Quay lại</a>
            </form>
        </div>
        <div class="form-group">
            <br>
